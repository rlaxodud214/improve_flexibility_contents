 ...
 * Unity object.
 */
public class MonsterShot_OpenZenMoveObject : MonoBehaviour
{
    /*
    데이터는 시작시 오프셋값 빼고 (0, 0, 0)에서 시작함 데이터 범위는 0~360도임

    float data = MonsterShot_OpenZenMoveObject.Instance.sensorEulerData.x; // 비틀기 - 카약, 담당자 : 노승재
    float data = MonsterShot_OpenZenMoveObject.Instance.sensorEulerData.y; // 굴곡/신전 - 림보, 담당자 : 오승연,박소윤
    float data = MonsterShot_OpenZenMoveObject.Instance.sensorEulerData.z; // 편측 굴곡/신전 - 골키퍼, 담당자 : 김희영
     */

    #region Singleton                                         // 싱글톤 패턴은 하나의 인스턴스에 전역적인 접근을 시키며 보통 호출될 때 인스턴스화 되므로 사용하지 않는다면 생성되지도 않습니다.
    private static MonsterShot_OpenZenMoveObject _Instance;          // 싱글톤 패턴을 사용하기 위한 인스턴스 변수, static 선언으로 어디서든 참조가 가능함
    public static MonsterShot_OpenZenMoveObject Instance                  // 객체에 접근하기 위한 속성으로 내부에 get set을 사용한다.
    {
        get                       // _sceneManager이 변수값을 리턴받을 수 있음.
        {
            if (_Instance == null)
                _Instance = FindObjectOfType<MonsterShot_OpenZenMoveObject>();
            return _Instance;
        }
    }
    #endregion
